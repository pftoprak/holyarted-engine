...
export function Experience({ user, signInPath, signOutPath }: ExperienceProps) {
  ...
  return (
    <main onPointerMove={(event) => { event.currentTarget.style.setProperty('--pointer-x', `${event.clientX}px`); event.currentTarget.style.setProperty('--pointer-y', `${event.clientY}px`); }} className="premium-canvas min-h-screen overflow-hidden bg-[#f1ede4] text-[#151816]">
      ...
      <section id="method" className="px-5 py-20 md:px-10 md:py-28"><div className="mx-auto max-w-[1400px]"><p className="eyebrow text-[#93683f]">{text.methodLabel}</p><h2 className="mt-5 max-w-4xl font-heading text-5xl leading-[0.95] tracking-[-0.04em] md:text-7xl">{text.methodTitle}</h2><div className="mt-14 grid border-y border-[#151816]/20 md:grid-cols-3">{text.principles.map(([title, body], index) => <article key={title} className={`py-8 md:min-h-64 md:p-8 ${index < 2 ? 'border-b border-[#151816]/15 md:border-b-0 md:border-r' : ''}`}><span className="block h-px w-12 bg-[#93683f]" aria-hidden="true" /><h3 className="mt-12 font-heading text-3xl">{title}</h3><p className="mt-4 text-sm leading-7 text-[#151816]/58">{body}</p></article>)}</div><p className="mt-8 max-w-3xl text-xs leading-6 text-[#151816]/45">{text.disclaimer}</p></div></section>

      <footer className="border-t border-[#151816]/15 px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1400px] flex-col gap-4 text-[10px] font-bold uppercase tracking-[0.16em] text-[#151816]/50 sm:flex-row sm:items-center sm:justify-between"><span>© HOLYARTED</span><span>{text.footer}</span>{user && <a href={signOutPath} target="_top" className="inline-flex items-center gap-2 hover:text-[#151816]"><LogOut className="size-3.5" />{text.signOut}</a>}</div></footer>
    </main>
  );
}
