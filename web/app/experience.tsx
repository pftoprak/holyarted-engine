...
export function Experience({ authConfig }: ExperienceProps) {
  ...
  return (
    <main onPointerMove={(event) => { event.currentTarget.style.setProperty('--pointer-x', `${event.clientX}px`); event.currentTarget.style.setProperty('--pointer-y', `${event.clientY}px`); }} className="premium-canvas min-h-screen overflow-hidden bg-[#f7f3ec] text-[#514b46]">
      ...
      <section id="profile" className="scroll-mt-20 bg-[#dcc4b8] px-5 py-20 md:px-10 md:py-28">
        <div className="mx-auto max-w-[1400px]">
          {complete ? <div>
            ...
            <div className="mt-10 flex flex-col items-start justify-between gap-6 border-t border-[#625b55]/22 pt-8 sm:flex-row sm:items-center"><button onClick={() => { setComplete(false); begin(); }} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em]"><ArrowLeft className="size-4" />{text.restart}</button>{user ? <Link href="/profile" className="inline-flex h-12 items-center gap-2 bg-[#625b55] px-6 text-xs font-bold text-[#f7f3ec]">{text.saveIn}<ArrowRight className="size-4" /></Link> : <button onClick={signInWithGoogle} className="inline-flex h-12 items-center gap-2 bg-[#625b55] px-6 text-xs font-bold text-[#f7f3ec]"><span className="grid size-5 place-items-center rounded-full bg-white text-[11px] font-black text-[#514b46]">G</span>{text.saveOut}</button>}</div>
      ...
      <section className="bg-[#bfc8ba] px-5 py-20 text-[#514b46] md:px-10 md:py-28"><div className="mx-auto grid max-w-[1400px] gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><p className="eyebrow text-[#7d695d]">{text.savePrompt}</p><h2 className="mt-5 max-w-3xl font-heading text-5xl leading-[0.95] md:text-7xl">{user ? `${text.account}: ${user.displayName}` : text.savePrompt}</h2><p className="mt-6 max-w-xl leading-7 text-[#514b46]/58">{text.saveCopy}</p></div>{user ? <Link href="/profile" className="premium-button lg:justify-self-end">{text.saveIn}<ArrowRight className="size-4" /></Link> : <button onClick={signInWithGoogle} className="premium-button lg:justify-self-end"><span className="grid size-5 place-items-center rounded-full bg-white text-[11px] font-black text-[#514b46]">G</span>{text.saveOut}</button>}</div></section>
      ...
      <footer className="border-t border-[#625b55]/12 px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1400px] flex-col gap-4 text-[10px] font-bold uppercase tracking-[0.16em] text-[#514b46]/50 sm:flex-row sm:items-center sm:justify-between"><span>© HOLYARTED</span><div className="flex flex-wrap gap-5"><Link href="/privacy" className="hover:text-[#514b46]">Privacy</Link><Link href="/terms" className="hover:text-[#514b46]">Terms</Link><a href="mailto:support@holyarted.com" className="hover:text-[#514b46]">Support</a></div><span>{text.footer}</span>{user && <button onClick={signOut} className="inline-flex items-center gap-2 hover:text-[#514b46]"><LogOut className="size-3.5" />{text.signOut}</button>}</div></footer>
    </main>
  );
}
