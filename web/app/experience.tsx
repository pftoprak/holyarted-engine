...
const copy = {
  en: {
    nav: ['Discover', 'Your portrait', 'Approach'], signIn: 'Sign in', account: 'Account', signOut: 'Sign out', menu: 'Menu',
    ...
    restart: 'Create another portrait', savePrompt: 'Keep your portrait close.', saveCopy: 'Sign in to return to your private space and continue your reflection over time.', saveIn: 'Open my account', saveOut: 'Sign in with ChatGPT',
  ...
  tr: {
    nav: ['Keşfet', 'Portren', 'Yaklaşım'], signIn: 'Giriş yap', account: 'Hesap', signOut: 'Çıkış yap', menu: 'Menü',
    ...
    restart: 'Başka bir portre oluştur', savePrompt: 'Portreni yanında tut.', saveCopy: 'Özel alanına dönmek ve zaman içindeki keşfine devam etmek için giriş yap.', saveIn: 'Hesabımı aç', saveOut: 'ChatGPT ile giriş yap',
...
export function Experience({ user, signInPath, signOutPath }: ExperienceProps) {
  ...
  return (
    <main onPointerMove={(event) => { event.currentTarget.style.setProperty('--pointer-x', `${event.clientX}px`); event.currentTarget.style.setProperty('--pointer-y', `${event.clientY}px`); }} className="premium-canvas min-h-screen overflow-hidden bg-[#eee8df] text-[#17141c]">
      <nav className="relative z-50 border-b border-white/10 bg-[#0d0d12] px-5 text-[#f5f0e8] md:px-10">
        <div className="mx-auto flex h-[76px] max-w-[1400px] items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="Holyarted home"><span className="grid size-9 place-items-center border border-[#c8a77a]/65 font-heading text-base text-[#d9bb91]">H</span><span className="font-heading text-sm tracking-[0.24em]">HOLYARTED</span></a>
          <div className="hidden items-center gap-8 lg:flex"><a href="#assessment" className="nav-link">{text.nav[0]}</a><a href="#profile" className="nav-link">{text.nav[1]}</a><a href="#method" className="nav-link">{text.nav[2]}</a><div className="flex items-center border-l border-white/15 pl-5 text-[11px] font-bold tracking-[0.12em]"><button onClick={() => setLocale('en')} className={locale === 'en' ? 'text-[#d9bb91]' : 'text-white/50'}>EN</button><span className="mx-2 text-white/25">/</span><button onClick={() => setLocale('tr')} className={locale === 'tr' ? 'text-[#d9bb91]' : 'text-white/50'}>TR</button></div><a href={user ? '/profile' : signInPath} target={user ? undefined : '_top'} className="inline-flex h-10 items-center gap-2 border border-white/25 px-5 text-xs font-bold transition hover:border-[#d9bb91] hover:text-[#d9bb91]">{user ? <UserRound className="size-4" /> : <LogIn className="size-4" />}{user ? text.account : text.signIn}</a></div>
        ...
        {menuOpen && <div className="border-t border-white/10 pb-6 pt-4 lg:hidden"><div className="mx-auto grid max-w-[1400px] gap-4 text-sm"><a href="#assessment" onClick={() => setMenuOpen(false)}>{text.nav[0]}</a><a href="#profile" onClick={() => setMenuOpen(false)}>{text.nav[1]}</a><a href="#method" onClick={() => setMenuOpen(false)}>{text.nav[2]}</a><div className="flex gap-4"><button onClick={() => setLocale('en')} className={locale === 'en' ? 'text-[#d9bb91]' : ''}>EN</button><button onClick={() => setLocale('tr')} className={locale === 'tr' ? 'text-[#d9bb91]' : ''}>TR</button></div><a href={user ? '/profile' : signInPath} target={user ? undefined : '_top'}>{user ? text.account : text.signIn}</a></div></div>}
      </nav>

      <section id="top" className="hero-stage relative bg-[#0d0d12] px-5 pb-20 pt-14 text-[#f5f0e8] md:px-10 md:pb-28 md:pt-24">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#c8a77a]/65 to-transparent" />
        <div className="hero-orbit hero-orbit-one" /><div className="hero-orbit hero-orbit-two" />
        <div className="mx-auto grid max-w-[1400px] gap-14 xl:grid-cols-[0.9fr_1.1fr] xl:items-end">
          <div className="max-w-3xl">
            <p className="eyebrow reveal-up text-[#d9bb91]">{text.edition}</p>
            <h1 className="mt-7 font-heading text-[clamp(4rem,8.2vw,8.1rem)] font-medium leading-[0.84] tracking-[-0.062em]">
              <span className="word-reveal block">{text.titleA}</span><span className="word-reveal word-reveal-delay block italic text-[#d9bb91]">{text.titleB}</span>
            </h1>
            ...
            <div className="mt-12 flex flex-wrap gap-x-7 gap-y-3 border-t border-white/10 pt-6">{text.trust.map((item) => <span key={item} className="inline-flex items-center gap-2 text-xs text-white/55"><Check className="size-3.5 text-[#d9bb91]" />{item}</span>)}</div>
          </div>
          <div id="assessment" className="interactive-panel relative scroll-mt-28 overflow-hidden border border-white/15 bg-[#18131b] shadow-[0_36px_90px_rgba(0,0,0,.36)]">
            <div className="flex items-center justify-between border-b border-white/10 px-6 py-5 md:px-8"><p className="text-[10px] font-bold tracking-[0.18em] text-[#d9bb91]">{text.panelLabel}</p><ShieldCheck className="size-4 text-white/35" /></div>
            <form onSubmit={calculate} className="relative p-6 md:p-9">
              ...
              <div className="mt-8">
                <div className="mb-3 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.14em] text-white/40"><span>{text.progress}</span><span>{text.inputLabel}</span></div>
                <div className="h-px bg-white/10"><div className="h-px w-full bg-[#d9bb91]" /></div>
                <div className="mt-8 grid gap-5 sm:grid-cols-2">
                  <label className="grid gap-2 text-xs font-bold text-white/65">{text.firstName}<input value={firstName} onChange={(event) => setFirstName(event.target.value)} placeholder={text.firstPlaceholder} autoComplete="name" className="h-14 border border-white/15 bg-white/[0.03] px-4 text-base font-normal text-[#f5f0e8] outline-none transition placeholder:text-white/25 focus:border-[#d9bb91]" required /></label>
            ...
            {isCalculating && <div className="calculation-overlay absolute inset-0 z-20 grid place-items-center bg-[#17111a]/96 p-8 text-center" aria-live="polite"><div className="w-full max-w-md"><div className="mx-auto grid size-20 place-items-center border border-[#d9bb91]/35"><span className="calculation-monogram font-heading text-4xl text-[#d9bb91]">H</span></div><p className="mt-8 font-heading text-4xl text-[#f5f0e8]">{text.calculating}</p><p className="mt-3 text-sm text-white/42">{text.calculatingNote}</p><div className="mt-9 h-px overflow-hidden bg-white/10"><div className="calculation-scan h-px bg-[#d9bb91]" /></div></div></div>}
          </div>
        </div>
      </section>

      <div className="marquee-shell border-y border-[#17141c]/15 bg-[#c8a77a] py-4"><div className="marquee-track text-[10px] font-bold uppercase tracking-[0.28em]"><span>{locale === 'en' ? 'SELF KNOWLEDGE · DECISION STYLE · CHARACTER · RELATIONSHIPS · PURPOSE · ' : 'ÖZ FARKINDALIK · KARAR BİÇİMİ · KARAKTER · İLİŞKİLER · AMAÇ · '}</span><span aria-hidden="true">{locale === 'en' ? 'SELF KNOWLEDGE · DECISION STYLE · CHARACTER · RELATIONSHIPS · PURPOSE · ' : 'ÖZ FARKINDALIK · KARAR BİÇİMİ · KARAKTER · İLİŞKİLER · AMAÇ · '}</span></div></div>

      <section id="preview" className="view-reveal px-5 py-20 md:px-10 md:py-28"><div className="mx-auto grid max-w-[1400px] gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-start"><div className="lg:sticky lg:top-28"><p className="eyebrow text-[#7a455e]">{text.previewLabel}</p><h2 className="mt-5 max-w-lg font-heading text-5xl leading-[0.95] tracking-[-0.04em] md:text-7xl">{text.previewTitle}</h2><p className="mt-6 max-w-md leading-7 text-[#17141c]/60">{text.previewCopy}</p><div className="image-lift mt-9 overflow-hidden border border-[#17141c]/15"><Image src="/og.png" width={1200} height={630} alt="Holyarted editorial identity" className="w-full transition duration-700 hover:scale-[1.025]" /></div></div><div><p className="mb-4 text-[10px] font-bold uppercase tracking-[0.18em] text-[#17141c]/38">{text.focusHint}</p><div className="grid border-t border-[#17141c]/20">{text.sections.map((label, index) => <button type="button" onClick={() => setActiveCard(activeCard === index ? null : index)} key={label} className={`insight-row grid gap-3 border-b border-[#17141c]/15 py-8 text-left sm:grid-cols-[9rem_1fr] sm:gap-8 ${activeCard === index ? 'is-active' : ''}`}><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a455e]">{label}</p><div><h3 className="font-heading text-3xl">{resultCards[index][0]}</h3><p className="mt-3 max-w-2xl text-sm leading-7 text-[#17141c]/58">{resultCards[index][1]}</p><span className="insight-line mt-6 block h-px w-0 bg-[#7a455e]" /></div></button>)}</div></div></div></section>

      <section id="profile" className="scroll-mt-20 bg-[#c8a77a] px-5 py-20 md:px-10 md:py-28"><div className="mx-auto max-w-[1400px]">{complete ? <div><div className="grid gap-10 border-b border-[#17141c]/25 pb-14 lg:grid-cols-[0.65fr_1.35fr]"><div><p className="eyebrow">{text.profileLabel}</p><p className="mt-4 text-xs text-[#17141c]/55">{text.profileFor}</p><p className="mt-2 font-heading text-2xl">{profile.fullName}</p></div><div><h2 className="font-heading text-6xl leading-[0.9] tracking-[-0.05em] md:text-8xl">{text.profileNames[profile.purpose]}</h2><p className="mt-7 max-w-3xl text-lg leading-8 text-[#17141c]/70">{text.profileIntros[profile.purpose]}</p></div></div><div className="grid md:grid-cols-2">{resultCards.map(([title, body], index) => <article key={title} className={`profile-result-card min-h-72 border-b border-[#17141c]/20 py-9 md:p-9 ${index % 2 === 0 ? 'md:border-r' : ''}`} style={{ animationDelay: `${index * 110}ms` }}><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#17141c]/55">{text.sections[index]}</p><h3 className="mt-10 font-heading text-4xl">{title}</h3><p className="mt-4 max-w-xl text-sm leading-7 text-[#17141c]/68">{body}</p></article>)}</div><div className="mt-10 flex flex-col items-start justify-between gap-6 border-t border-[#17141c]/25 pt-8 sm:flex-row sm:items-center"><button onClick={() => { setComplete(false); begin(); }} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em]"><ArrowLeft className="size-4" />{text.restart}</button><a href={user ? '/profile' : signInPath} target={user ? undefined : '_top'} className="inline-flex h-12 items-center gap-2 bg-[#0d0d12] px-6 text-xs font-bold text-[#f5f0e8]">{user ? text.saveIn : text.saveOut}<ArrowRight className="size-4" /></a></div></div> : <div className="grid items-end gap-10 lg:grid-cols-2"><div><p className="eyebrow">{text.profileLabel}</p><h2 className="mt-6 font-heading text-6xl leading-[0.9] tracking-[-0.045em] md:text-8xl">{text.profileNames[profile.purpose]}</h2></div><div className="max-w-xl lg:justify-self-end"><p className="text-lg leading-8 text-[#17141c]/70">{text.profileIntros[profile.purpose]}</p><button onClick={begin} className="mt-8 inline-flex h-12 items-center gap-2 bg-[#0d0d12] px-6 text-xs font-bold text-[#f5f0e8]">{text.start}<ArrowRight className="size-4" /></button></div></div>}</div></section>

      <section className="bg-[#2a1824] px-5 py-20 text-[#f5f0e8] md:px-10 md:py-28"><div className="mx-auto grid max-w-[1400px] gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><p className="eyebrow text-[#d9bb91]">{text.savePrompt}</p><h2 className="mt-5 max-w-3xl font-heading text-5xl leading-[0.95] md:text-7xl">{user ? `${text.account}: ${user.displayName}` : text.savePrompt}</h2><p className="mt-6 max-w-xl leading-7 text-white/50">{text.saveCopy}</p></div><a href={user ? '/profile' : signInPath} target={user ? undefined : '_top'} className="premium-button lg:justify-self-end">{user ? text.saveIn : text.saveOut}<ArrowRight className="size-4" /></a></div></section>

      <section id="method" className="px-5 py-20 md:px-10 md:py-28"><div className="mx-auto max-w-[1400px]"><p className="eyebrow text-[#7a455e]">{text.methodLabel}</p><h2 className="mt-5 max-w-4xl font-heading text-5xl leading-[0.95] tracking-[-0.04em] md:text-7xl">{text.methodTitle}</h2><div className="mt-14 grid border-y border-[#17141c]/20 md:grid-cols-3">{text.principles.map(([title, body], index) => <article key={title} className={`py-8 md:min-h-64 md:p-8 ${index < 2 ? 'border-b border-[#17141c]/15 md:border-b-0 md:border-r' : ''}`}><span className="block h-px w-12 bg-[#7a455e]" aria-hidden="true" /><h3 className="mt-12 font-heading text-3xl">{title}</h3><p className="mt-4 text-sm leading-7 text-[#17141c]/58">{body}</p></article>)}</div><p className="mt-8 max-w-3xl text-xs leading-6 text-[#17141c]/45">{text.disclaimer}</p></div></section>

      <footer className="border-t border-[#17141c]/15 px-5 py-8 md:px-10"><div className="mx-auto flex max-w-[1400px] flex-col gap-4 text-[10px] font-bold uppercase tracking-[0.16em] text-[#17141c]/50 sm:flex-row sm:items-center sm:justify-between"><span>© HOLYARTED</span><span>{text.footer}</span>{user && <a href={signOutPath} target="_top" className="inline-flex items-center gap-2 hover:text-[#17141c]"><LogOut className="size-3.5" />{text.signOut}</a>}</div></footer>
    </main>
  );
}
