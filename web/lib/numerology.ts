...
export function reduceNumber(total: number) {
  let value = total;
...
function sumName(name: string) {
  return [...name.toLocaleUpperCase('tr-TR')].reduce((sum, letter) => sum + (letterValues[letter] ?? 0), 0);
}
...
export function calculateProfile(name: string, birthDate: string) {
  ...
  const expressionNumber = reduceNumber(sumName(name));
  ...
  return {
    ...
    directionNumber,
    sections: [
      { key: 'essence', eyebrow: 'Öz enerji', title: 'Dünyaya nasıl görünüyorsun?', number: lifeNumber, content: pick(showcase as ContentMap, lifeNumber) },
      { key: 'gift', eyebrow: 'Doğal yetenek', title: 'Sende zahmetsizce akan güç', number: expressionNumber, content: pick(abilities as ContentMap, expressionNumber) },
      { key: 'work', eyebrow: 'Üretim yönü', title: 'Potansiyelini nerede büyütürsün?', number: directionNumber, content: pick(idealJobs as ContentMap, directionNumber) },
    ],
  };
}

export type HolyartedProfile = ReturnType<typeof calculateProfile>;
