...
function sumName(name: string) {
  return Array.from(name.toLocaleUpperCase('tr-TR')).reduce((sum, letter) => sum + (letterValues[letter] ?? 0), 0);
}

function pick(map: ContentMap, key: number) {
...
function mappedKey<T>(keys: T[], value: number) {
  return keys[(reduceNumber(value) - 1 + keys.length) % keys.length];
}
